...
# 该脚本用于处理经 UDG 处理或现代双端数据的 paired 部分，与参考基因组比对、过滤
# 需要准备 INPUT_LIST 文件：
# ERR4658035
# ERR4659298
...
do
    ...
    echo "--- Processing SAMEA7368724: ${run_id} ---"
    
    # 定义中间和最终文件路径
    INPUT_BAM="${run_id}.paired.bam"
    SORTED_BAM="${run_id}.paired.ARS1.mapping.sorted.bam" # 中间文件
    FINAL_BAM="${run_id}.paired.ARS1.mapping.sorted.MQ30.bam"
    ...
    samtools view -h -b -q 30 -f 2 "${SORTED_BAM}" -o "${FINAL_BAM}"
